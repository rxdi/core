...
describe('Service: Metadata', () => {
  it('Should create appropriate metadata based on imports inside module and hash should equal 32 signs', done => {
    ...
    expect(generatedHashData[0][0]['hash']).toBe(
      PeshoService['metadata']['moduleHash']
    );
    expect(generatedHashData[0][0]['moduleName']).toBe(
      PeshoService['metadata']['moduleName']
    );
    expect(generatedHashData[1][0]['hash']).toBe(
      PeshoModule['metadata']['moduleHash']
    );
    expect(generatedHashData[1][0]['moduleName']).toBe(
      PeshoModule['metadata']['moduleName']
    );
